...
namespace Jobs
{
    ...
    class HistoricJobsDateOfInterest
    {
        //Access Credentials
        ...
        private static void GetLatestJobs(Object source, ElapsedEventArgs e)
        {
            ...
            //To find jobs that were completed on a specific date, we go thru the current job set and compare the
            //jobCompleteTime and add it to a new list if the date matches the date of interest. In this case, we 
            //are looking for jobs that completed 3 days ago.
            List<HistoricJob> jobsCompletedOnDateOfInterest = new List<HistoricJob>();
            DateTime dateOfInterest = DateTime.Now;
            dateOfInterest.AddDays(-3);
            foreach (HistoricJob job in jobs)
            {
                DateTime jobCompleteTime = DateTime.Parse(job.jobCompleteTime);
                if (jobCompleteTime.Date == dateOfInterest.Date)
                {
                    jobsCompletedOnDateOfInterest.Add(job);
                }
            }

            Console.WriteLine("Looking for Jobs Completed on " + dateOfInterest.ToString("dd/MM/yyyy") + "\n" +
                "Total Jobs Completed on " + dateOfInterest.ToString("dd/MM/yyyy") + ": " + jobsCompletedOnDateOfInterest.Count);
