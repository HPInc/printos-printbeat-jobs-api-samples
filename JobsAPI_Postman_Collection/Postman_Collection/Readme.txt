...
Import the "Jobs API.postman_collection.json" file in the main Import dialog of Postman.
Import the "Jobs API Environment.postman_environment.json" file through the Manage Environments dialog in the upper right settings menu

In the Manage Environments dialog edit the environment you just imported and replace the "key" and "secret" values with the key and secret from the PrintOS account you are using. 
Make sure the SmsHost enviroment variable is checked for the stack you are using (Production vs Staging)
Click "Update" to save changes.

Select the Environment you just imported in the environment drop-down menu.
In the Jobs API collection in the left pane select the API call you wish to make then click the "Send" button to send the API call.
